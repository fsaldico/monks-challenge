...
-- Conteo de registros
SELECT '1. Total registros' AS metrica, COUNT(*) AS valor FROM `mm-tse-latam-interviews.challange_florencia.VENTAS_LIMPIA`;

-- Rango de fechas
SELECT '2. Rango fechas' AS metrica, 
  CONCAT(CAST(MIN(CREATION_DATE) AS STRING), ' - ', CAST(MAX(CREATION_DATE) AS STRING)) AS valor
